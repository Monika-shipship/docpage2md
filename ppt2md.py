...
# --- 阶段一：视觉提取 Prompt (习题识别版) ---
PROMPT_STAGE_1_VISION = r"""
任务：请对这张图片中的习题进行精准提取。

【提取要求】
1. **题目识别**：精准识别题目文本，区分题干、选项（如果有）、小问。
2. **公式识别**：所有数学/物理公式转换为 LaTeX 格式。
3. **图像描述**：如果题目包含插图（几何图、函数图、电路图等），请详细描述图像内容，包括形状、标注、坐标轴、关键点等，以便于理解题目。
4. **排版保持**：保留题目原有的结构（如填空题的下划线、选择题的列表）。
...
你是一位专业的学科辅导老师。你的任务是根据提供的习题识别内容，整理题目并提供详细的解析。
...
1. **题目整理**：将识别到的文本整理为清晰的 Markdown 格式。
   - 确保公式使用标准的 LaTeX 格式：行内公式 $...$，行间公式 $$...$$。
   - 修正 OCR 可能产生的明显错误。
2. **答案提取**：明确题目的最终答案（如选项字母、数值结果、证明结论）。
3. **题目解析**：根据题目内容，提供详细的解题思路和步骤。
   - 如果是选择题，请给出正确选项并解释原因。
   - 如果是计算题，请给出完整的计算过程。
   - 如果是证明题，请给出严谨的证明逻辑。
...
## 📝 题目
(在此处输出整理好的题目内容，包括题干、选项、图示描述等)

## 🗝️ 答案
(在此处仅输出最终答案。例如：**A** 或 **x = 5**)
...
def run_stage_2_brain_parallel(slide_no, ppt_root, raw_data_map):
    """
    Step 2 Worker: 单页 Raw Data -> 调用 Brain (流式版)
    """
    # 1. 准备数据 (仅当前页)
