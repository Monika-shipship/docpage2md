...
5. 模型档位选“均衡（推荐）”。
6. MinerU 模型保持默认 `vlm`。
7. 页码范围先填 `1-3` 做小样本测试；确认效果后留空跑全量。
8. 点击“刷新估算”，查看 Vision/Brain 费用估算。
9. 点击“开始处理”。
...
- 成本估算、命令预览、中文日志、进度条、ETA、放大日志、打开输出目录。
...
GUI 当前主推 MinerU/PaddleOCR 解析主路径。`vision_only` 保留在 CLI 中。

文档类型只是推荐预设，不会锁死旁边选项。你可以先选“手写矢量笔记”，再手动改处理模式、模型档位、MinerU 模型版本或具体模型。
...
`dual_hybrid` 会对同一个本地文件同时调用 MinerU 和 PaddleOCR。当前策略是 MinerU 作为主版面和 crop 骨架，PaddleOCR 作为同页 OCR/layout 证据，Brain 精修阶段再用两份证据做局部纠错。它更慢、会占用两个解析平台额度，但适合手写公式、复杂 OCR 或两套解析结果差异明显的页面。

当前首版支持：

- 本地单文件、多文件、文件夹。
- 已有 artifact：同时提供 `--mineru-artifact-dir` 和 `--paddleocr-artifact-dir`。
- 输出同时保留 `mineru_raw/`、`paddleocr_raw/`、融合后的 `ir/` 和 `run_report.json`。
