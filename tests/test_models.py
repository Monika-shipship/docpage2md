...
def test_stage2_returns_structured_error(monkeypatch):
    monkeypatch.setattr(models, "_run_deepseek_brain", lambda filled_prompt, config: "Brain Error: 500")
...
def test_stage2_returns_sanitized_markdown(monkeypatch):
    monkeypatch.setattr(models, "_run_deepseek_brain", lambda filled_prompt, config: "正文")
