...
-   **思考过程：** 阿里云百炼提供混合思考和仅思考两种视觉推理模型。
    ...
    -   **混合思考模型：**可通过`enable_thinking`控制其思考行为：
        ...
        -   设置为 `true`，开启思考，模型将先输出思考过程，再输出最终回复。Qwen3.6、Qwen3.5系列模型默认为`true`。
...
-   **输出方式：**视觉推理模型包含详细的思考过程，为避免因响应内容过长导致超时，建议使用流式输出。
    ...
    -   Qwen3.6、Qwen3.5、Qwen3-VL、kimi-k2.6、kimi-k2.5、stepfun/step-3.7-flash系列支持**流式和非流式**两种方式。
...
`thinking_budget` 参数支持Qwen3.6、Qwen3.5、Qwen3-VL（思考模式）、kimi-k2.6、kimi-k2.5（思考模式）。
...
关于视觉推理模型的输入输出参数，请参见[文本生成](https://help.aliyun.com/zh/model-studio/qwen-api-reference/)。
