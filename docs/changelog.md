...
- 运行页不再使用整页纵向滚动，改为固定双栏工作台：左侧配置页签、右侧日志/成本页签、底部固定操作栏。
- “开始处理 / 停止 / 打开输出目录”固定在运行页底部，最小窗口尺寸下仍可见。
- 左侧配置拆分为“工作流 / 输入文件 / 输出 / 引擎高级 / 并发/Brain”页签，避免输出与并发设置在非全屏窗口下互相遮挡。
- 工作流控件改为两列字段卡片，下拉框获得完整宽度，减少文字被截断。
- 新增 GUI 回归测试，断言最小窗口尺寸下开始处理按钮不会被内容顶出窗口。
