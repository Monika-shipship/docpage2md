...
- Figure details renderer now repairs adjacent inline math delimiters such as `$\\Sigma$$J \\neq 0$` before writing `<details>`, preventing false `display_math_unbalanced` / `inline_math_unbalanced` fail-open pages.
