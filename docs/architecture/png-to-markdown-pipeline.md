# PNG 到 Markdown 的生成原理

本文说明 DocPage2MD 当前从单页 PNG/JPG 文档图片生成 Markdown 的核心流程。这里的重点不是让模型自由“看图写作文”，而是把识别、结构化、重组、校验和保守回退拆成可审计的流水线，尽量避免公式、图表、手写文字等内容被静默遗漏。

## 目标

输入是一组文档页图片，例如扫描页、PDF 导出的 PNG、PPT 导出的图片、手写笔记截图。输出仍然是用户直接使用的 Markdown：
...
> 原因：target_formula_block_missing: ...
```
...
- 不可靠公式渲染为 warning block，并保留原始识别
- 可靠表格渲染为 Markdown table
- 不可靠表格渲染为 warning + fenced raw text，有图片证据时保留图片引用
- 图示渲染为 `> [!NOTE] 图示说明` 或 `> [!WARNING] 图示识别不确定`
- 不确定内容渲染为 `> [!WARNING] 识别不确定`
