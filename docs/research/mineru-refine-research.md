...
报告输出位置：`D:\Repos\lab-python\png2md\docs\research\mineru-refine-research.md`

研究方式：静态阅读仓库源码、README、绑定、脚本、plugin 与测试文件。本文只记录已从源码或项目文件中验证的事实；对 `png2md` 的部分为迁移分析与建议，不把建议写成已实现事实。
...
对 `png2md` 最有价值的迁移点不是照搬 Rust 实现，而是把“结构化中间结果 + 受限修复操作 + 保真/审计闸门 + fail-open”的设计移植到现有 Python 应用里。这样能在不破坏主流程的前提下增加一个后处理层，降低 OCR/解析后处理引入不可控重写的风险。
...
这对 `png2md` 很有参考价值，因为 `png2md` 当前是 Python 项目。如果引入类似设计，优先级应是“用 Python 实现同类后处理接口”，而不是立刻引入 PyO3/Rust 复杂度。
...
这个文件是“数据契约中心”，也是后续迁移到 `png2md` 时应先设计的部分：先定义中间 item、suspect、op、report，再写具体检测器和执行器。
...
这个层不是默认核心清洗，而是可选增强。对 `png2md` 来说，OCR 纠错通常有价值，但必须有白名单和 provenance，否则很容易把正确文本改错。
...
这个模块对 MinerU 表格场景高度定制。对 `png2md` 的迁移价值取决于 `png2md` 是否有表格图片裁剪和结构化表格中间表示。
...
- 高。`png2md` 如果已有自己的 page/block/span/table 中间结构，后处理层最好保持输入输出结构兼容，不应该让清洗器改变主流程的数据契约。
...
- 中高。若 `png2md` 引入 LLM 后处理，建议先做单轮 suspect 裁决，再逐步演进到完整 tool-use loop。
...
- 高，但需要分层。`png2md` 如果要从图片生成描述，严格 no-new-char 不适用，因为 OCR/视觉描述本来会生成新文本。
- 建议采用三类合同：
  1. 结构清洗层：不新增内容字符。
...
- 高。`png2md` 若处理多页图片、PPT 或 PDF，progress 应成为统一接口，而不是散落 print。
...
- `png2md` 也应把“本地测试、真实样本 smoke、端到端导出、报告生成”固化为脚本，而不是依赖人工命令记忆。
...
- 如果 `png2md` 引入可选模型后处理，应提供 offline audit 和 smoke 脚本，分别验证“无需网络的审计”和“真实模型连接是否可用”。
...
- 对 `png2md` 而言，如果要做中文 OCR 纠错或乱码检测，词典生成脚本比手工维护大列表更可靠。
...
- `png2md` 当前作为 Python 项目，短期更应学习它的 API 形状，而不是学习 PyO3 本身。
...
- 如果 `png2md` 将来提供 Web/Node 插件生态，可参考。
- 当前优先级不高，除非已有 JS 消费方。
...
- `png2md` 应该先为 detector 写测试，再让 LLM 介入。检测器是可控性的来源。
...
- `png2md` 的 invariant 应该独立成模块，不要埋在 op 里。
...
- `png2md` 后处理也应有“第二次运行 no-op”的幂等性测试。
...
下表中的“适合度”是对当前 `png2md` 这类 Python 图片/文档转 Markdown 项目的判断，不表示 mineru-refine 已经在 `png2md` 中实现。

| 可迁移思想 | 解决什么问题 | 代价 | 是否适合 png2md |
|---|---|---|---|
...
| detector 先生成 suspect | 降低 LLM 上下文和误判，让问题定位可测试 | 要写规则和测试 | 高。png2md 应先规则定位 OCR/结构疑点 |
...
| `apply_op_checked` 单入口 | 所有改动统一校验、审计、回滚 | 初期架构约束更重 | 高。建议 png2md 也不要让代码到处直接改 block |
| 保真闸门 | 防止新增内容、表格破坏、几何错误 | 对生成式任务过于严格，需要分层 | 高，但要按 OCR/生成/清洗分不同合同 |
| table body 专门 invariant | 普通字符 multiset 不适合 HTML/表格结构 | 实现复杂，测试成本高 | 中高。若 png2md 输出表格，值得做 |
...
| progress 统一模型 | 长任务有反馈，多端一致 | CLI/Web/Python 要分别适配 | 高。png2md 多页处理需要 |
...
| opt-in garbled table rewrite | 用视觉模型救结构化 OCR 已坏的表格 | 需要表格图片裁剪、视觉模型、结构闸门 | 视情况。如果 png2md 有表格 crop，则适合；否则成本高 |
| garbled degrade to image | 表格无法可靠转写时保留视觉信息 | Markdown 体验下降，需图片路径管理 | 中高。比输出坏表格更安全 |
| deterministic Markdown renderer | Markdown 是结构派生产物，便于测试 | 需要维护 renderer | 高。png2md 应避免多个地方各自拼 Markdown |
| mock LLM tests | 测试稳定、无网络、低成本 | 需要抽象模型接口 | 极高。任何模型功能都应可 mock |
| idempotence 测试 | 防止二次 refine 继续破坏结果 | 需要稳定输出和缓存策略 | 高。后处理器应二次运行 no-op |
| plugin 式端到端包装 | 把库能力变成用户工作流 | 需要维护依赖检测、目录约定、报告输出 | 中。取决于 png2md 是否要做 Codex/Claude 插件 |
| Rust core + 多语言绑定 | 高性能、一套逻辑多端复用 | Rust/PyO3/napi 发布复杂 | 低到中。当前 png2md 是 Python 项目，短期不建议优先迁移到 Rust |
...
这部分要视 `png2md` 的真实输入和结构化程度决定，不宜在基础 refine 未稳定时提前做。

## 10. 不建议直接照搬的部分

### 10.1 不建议短期改成 Rust core

mineru-refine 的 Rust core 很完整，但它是为多语言分发和高可靠后处理设计的。`png2md` 当前工作区显示是 Python 项目，短期直接引入 Rust/PyO3 会带来：
...
如果 `png2md` 的结构不同，必须先做 schema mapping。不能把 `detect.rs` 里的规则机械翻译成 Python，否则会产生大量误报。

### 10.3 不建议默认打开生成式纠错

mineru-refine 对 core 层坚持“不新增一个字”，对 OCR confusion 和 garbled rewrite 也做成 opt-in，并记录 provenance。`png2md` 如果默认让 LLM 润色/补全文字，会显著提高幻觉风险。

建议默认只启用保守结构清理；生成式增强必须显式开启，并在输出中标识 generated 来源。

## 11. 可作为 png2md 设计参考的接口草案

下面是基于 mineru-refine 思想给 `png2md` 的 Python 侧接口草案，不表示当前仓库已经实现：
...
```json
{
  "logicVersion": "png2md-refine-0.1",
  "promptVersion": "p1",
...
4. `split_table` 视觉路径依赖 MinerU `img_path` 指向裁剪图。若 `png2md` 没有等价图片 crop，不能直接复用这类判断。
