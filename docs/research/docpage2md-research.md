...
生成时间：2026-06-19
研究者角色：Subagent B
工作区：`D:\Repos\lab-python\docpage2md`
当前分支：`codex/mineru-refine-inspired`
报告目标文件：`D:\Repos\lab-python\docpage2md\docs\research\docpage2md-research.md`

## 0. 研究边界与已验证事实

本报告基于当前工作区源码与运行产物的只读研究结果编写。除创建本报告所在目录与本 Markdown 文件外，未修改业务代码。

需要先说明一个命名事实：正式项目名已经统一为 `DocPage2MD`，正式入口是 `docpage2md.py`，实际 Python 包名是 `docpage2md_app`。用户要求研究的 `cli.py`、`pipeline.py`、`models.py`、`prompts.py`、`config.py`、`cost.py`、`files.py`、`model_settings.py`、`third_party_models.py` 都位于 `docpage2md_app\` 下。

当前 `git status --short --untracked-files=all` 验证到以下未提交改动：

```text
 M docpage2md_app/config.py
 M docpage2md_app/model_settings.py
?? docpage2md_app/third_party_models.py
```

其中 `third_party_models.py` 是未跟踪的新文件；`config.py` 和 `model_settings.py` 已修改。`git diff --stat` 显示已跟踪文件的差异为：

```text
 docpage2md_app/config.py         |   4 +
 docpage2md_app/model_settings.py | 476 +++++++++++++++++++++++++++++++++++++------
 2 files changed, 415 insertions(+), 65 deletions(-)
```

未跟踪文件 `docpage2md_app/third_party_models.py` 不在上述 diff stat 中，但已被当前 `model_settings.py` import，因此它是当前运行形态的一部分，不能只按已提交版本理解项目。

当前仓库未发现 `tests/` 目录；对仓库执行 `rg -n "unittest|pytest|def test_|class Test|assert " .` 没有匹配。当前也没有 `docs/` 目录，报告写入前由本次任务创建了 `docs\research`。

## 1. 项目当前目标与定位

项目目标是把 PDF、Office 文档、图片或旧版文档页图片目录批量转换为结构化 Markdown 笔记。当前新主路径通过 MinerU 直接处理 PDF、PPTX、Docx、Excel 和图片等多格式文件；旧版 `vision_only` 路径仍支持读取图片目录，例如 `doc_pages\某个任务\001.png`、`002.png` 等，然后输出单页 Markdown、视觉中间缓存和汇总 Markdown。

当前产品形态更接近一个交互式本地 CLI 批处理工具，而不是库或服务：

- 入口是仓库根目录的 `docpage2md.py`，它只负责调用 `docpage2md_app.cli.main()`。
- 主交互流程在 `docpage2md_app\cli.py:208` 的 `main()` 中编排。
- 用户通过 `python docpage2md.py` 启动，随后在终端中选择模型、选择图片任务、选择页码范围、查看成本预估并确认执行。
...
`docpage2md_app\cli.py:35` 的 `ensure_dependencies()` 会检查 `rich`、`PIL`、`dashscope` 是否可 import，缺少时直接提示安装包名。没有检测 Python 版本，也没有检测网络连通性、API Key 有效性或模型权限；API Key 和模型可用性在后续步骤处理。
...
`docpage2md_app\cli.py:12` 的 `parse_args()` 定义了以下主要参数：
...
`docpage2md_app\cli.py:208` 的 `main()` 是主流程入口。按源码顺序，普通转换模式执行：
...
6. `ensure_input_folder(config)` 检查 `doc_pages`，不存在则创建并要求用户放入图片后重跑。
...
10. `interactive_setup(console, final_key, config)` 扫描图片任务、处理断点会话、选择文档页图片任务和页码范围。
11. `show_cost_estimation(console, tasks_config, config)` 做成本预估。
12. 用户确认开始。
13. 创建 `multiprocessing.Manager().Queue()` 和 Rich `Progress`。
14. 用 `ProcessPoolExecutor(max_workers=config.max_docpage_workers)` 对多个文档页图片任务并行提交 `process_single_docpage_task()`。
...
默认路径在 `docpage2md_app\config.py` 中定义：

- `DEFAULT_INPUT_FOLDER = "./doc_pages"`，见 `docpage2md_app\config.py:11`。
- `DEFAULT_OUTPUT_FOLDER = "./markdown_output"`，见 `docpage2md_app\config.py:12`。
- `DEFAULT_LOG_FOLDER = "./log"`，见 `docpage2md_app\config.py:13`。
- `DEFAULT_MODEL_CACHE_PATH = ".cache/aliyun_model_catalog.json"`，见 `docpage2md_app\config.py:14`。

`AppConfig` 是 frozen dataclass，定义于 `docpage2md_app\config.py:41`。由于它是 frozen，模型选择和配置变更通常通过 `dataclasses.replace()` 返回新配置对象，而不是原地修改。

### 3.2 输入结构

输入扫描逻辑在 `docpage2md_app\files.py:123` 的 `scan_docpage_folders(root_folder)`：
...
README 中也明确要求先把 PPT/PDF 导出成图片后放入 `doc_pages\任务名\`。

### 3.3 输出结构

单个任务的输出根目录由 `docpage2md_app\pipeline.py:25` 计算：

```text
{config.output_folder}\{doc_name}
```

在该目录下，`process_single_docpage_task()` 会创建：

- `temp_raw_vision\`：Step 1 视觉模型 Raw Data 缓存目录。
- `Slide_XX.md`：Step 2 Brain 模型整理后的单页 Markdown。
- `{doc_name}_FULL.md`：由 `files.merge_markdowns()` 生成的汇总 Markdown。

Step 1 缓存文件名格式由 `docpage2md_app\pipeline.py:82` 定义：
...
Step 2 输出文件名格式由 `docpage2md_app\pipeline.py:158` 定义：
...
汇总逻辑在 `docpage2md_app\files.py:150` 的 `merge_markdowns()`：

- 查找输出目录下所有 `Slide_*.md`。
- 按自然排序读取。
- 写入 `{doc_name}_FULL.md`。
...
- 日志：`log\log_{session_name}_{YYYYMMDD_HHMMSS}.log`，由 `docpage2md_app\files.py:13` 的 `setup_logger()` 创建。
- 会话：`log\sessions\session_{session_name}.json`，由 `docpage2md_app\config.py:77` 和 `docpage2md_app\session.py:24` 控制。
- 旧会话兼容路径：仓库根目录下的 `session_{session_name}.json`，见 `docpage2md_app\config.py:85` 和 `docpage2md_app\session.py:7`。
- 模型偏好：`log\model_settings.json`，见 `docpage2md_app\config.py:105` 与 `docpage2md_app\model_settings.py:687`。
- 第三方模型注册表：`log\third_party_models.json`，这是当前未提交改动新增的路径，见 `docpage2md_app\config.py:109` 与 `docpage2md_app\third_party_models.py:20`。
- 阿里云模型目录缓存：`.cache\aliyun_model_catalog.json`，见 `docpage2md_app\config.py:14`、`docpage2md_app\aliyun_catalog.py:745`。
...
当前项目最核心的设计是 Vision/Brain 双阶段 pipeline。控制器位于 `docpage2md_app\pipeline.py:10` 的 `process_single_docpage_task()`。

### 4.1 Stage 1: Vision 视觉提取

Stage 1 的调度函数是 `docpage2md_app\pipeline.py:71` 的 `_run_vision_stage()`。它对当前任务选中的图片页执行视觉模型调用，产出 Raw Data。
...
- 如果缓存不存在或读取失败，则提交线程任务 `run_stage_1_vision(img_path, actual_slide_no, doc_name, msg_queue, config)`。
- 成功后把完整 result 写入 `Raw_XX.json`。
- 失败时把 `raw_data_map[slide_no]` 设置为字符串 `[Vision Failed]`，并继续流程。

真正调用视觉模型的函数是 `docpage2md_app\models.py:246` 的 `run_stage_1_vision()`：
...
- 视觉 prompt 来自 `docpage2md_app\prompts.py:1` 的 `PROMPT_STAGE_1_VISION`。
...
Stage 2 的调度函数是 `docpage2md_app\pipeline.py:139` 的 `_run_brain_stage()`。它把每一页的 Raw Data 与前后上下文送入 Brain 模型，生成最终 Markdown。
...
真正构造 Brain prompt 的函数是 `docpage2md_app\models.py:395` 的 `run_stage_2_brain_parallel()`。它把五页滑动窗口填入 `PROMPT_STAGE_2_BRAIN`：
...
窗口内容通过 `docpage2md_app\models.py:387` 的 `get_raw_content()` 获取，超过 3000 个字符会截断为前 3000 字符并追加 `...(truncated)`。

Brain prompt 定义于 `docpage2md_app\prompts.py:26`，其关键约束是：
...
- `deepseek`：调用 `_run_deepseek_brain()`，见 `docpage2md_app\models.py:525`。
- `dashscope_openai` 或 `openai_compatible`：调用 `_run_openai_compatible_brain()`，见 `docpage2md_app\models.py:501`。
- 其他默认走 DashScope 原生文本 API `_run_dashscope_brain()`，见 `docpage2md_app\models.py:464`。

最终结果会经过 `docpage2md_app\models.py:417` 的 `sanitize_stage_2_markdown()`：
...
顶层并发在 `docpage2md_app\cli.py:274` 使用 `ProcessPoolExecutor(max_workers=config.max_docpage_workers)`。旧版图片目录流程中，每个文档页图片文件夹作为一个独立任务提交给子进程执行 `process_single_docpage_task()`。

默认 `max_docpage_workers` 来自 `docpage2md_app\config.py:16`：

```text
DEFAULT_MAX_DOCPAGE_WORKERS = 1
```

这意味着默认不会同时处理多个文档页图片任务。README 也解释这是为了避免多个大任务同时调用 API 导致限流。
...
单个 DocPage2MD 图片任务内部，Stage 1 和 Stage 2 都使用 `ThreadPoolExecutor`：

- Stage 1：`docpage2md_app\pipeline.py:81`，`max_workers=config.vision_batch_workers`。
- Stage 2：`docpage2md_app\pipeline.py:153`，`max_workers=config.brain_batch_workers`。

默认 worker 数在 `docpage2md_app\config.py:17` 和 `docpage2md_app\config.py:18`：
...
- `docpage2md_app\models.py:26` 定义了 `retry_with_backoff()` 装饰器，但从当前源码看，主要 API 调用路径并未系统性套用该装饰器。
...
Stage 2 的断点判断在 `docpage2md_app\pipeline.py:160`：
...
会话逻辑在 `docpage2md_app\session.py`：

- `load_session()` 见 `docpage2md_app\session.py:7`，先查 `log\sessions\session_{name}.json`，再查旧路径。
- 如果读取到旧路径且新路径不存在，会调用 `save_session()` 迁移。
- `interactive_setup()` 见 `docpage2md_app\session.py:50`，扫描到旧会话后询问是否继续。
- `save_session()` 见 `docpage2md_app\session.py:24`，保存 `tasks_config`，包含 images 列表、range_start、range_end、task_id。
...
日志创建在 `docpage2md_app\files.py:13` 的 `setup_logger()`：

- 创建 `log` 目录。
- 日志文件名包含 session name 和时间戳。
- logger 名称是 `DocPage2MD_{session_name}`。
...
成本估算位于 `docpage2md_app\cost.py`，入口是 `show_cost_estimation()`，见 `docpage2md_app\cost.py:141`。

### 8.1 图片 token 估算

`calculate_image_tokens()` 见 `docpage2md_app\cost.py:4`。它使用 Qwen3-VL 规则近似计算图片 token：
...
`estimate_text_cost()` 见 `docpage2md_app\cost.py:59`：
...
如果找不到 `ModelRecord` 或价格，`show_cost_estimation()` 会 fallback 到 `estimate_price()`，见 `docpage2md_app\cost.py:24`。该 fallback 只覆盖少数旧模型，例如 `qwen3-vl-plus` 和 `qwen-plus`。

第三方或自定义模型价格通过 `config.vision_input_price_per_million`、`config.vision_output_price_per_million`、`config.brain_input_price_per_million`、`config.brain_output_price_per_million` 进入 `_find_model_record()`，见 `docpage2md_app\cost.py:246`。
...
`docpage2md_app\model_catalog.py` 定义静态精选目录：

- `ModelPrice` 见 `docpage2md_app\model_catalog.py:12`。
- `ModelSpec` 见 `docpage2md_app\model_catalog.py:22`。
- `get_default_catalog()` 见 `docpage2md_app\model_catalog.py:45`。
- `static_to_records()` 见 `docpage2md_app\model_catalog.py:368`，把静态目录转换成 `aliyun_catalog.ModelRecord`。
- `merge_static_and_dynamic()` 见 `docpage2md_app\model_catalog.py:471`，合并静态目录和动态缓存。
- `load_model_catalog()` 见 `docpage2md_app\model_catalog.py:575`，作为模型目录统一入口。
...
`docpage2md_app\aliyun_catalog.py` 负责动态抓取和验证：

- `ModelRecord` 数据结构见 `docpage2md_app\aliyun_catalog.py:227`。
- `fetch_model_ids_from_docs()` 见 `docpage2md_app\aliyun_catalog.py:274`，从公开阿里云文档页面提取模型 ID 并合并价格。
- `verify_openai_chat_model()` 见 `docpage2md_app\aliyun_catalog.py:510`。
- `verify_dashscope_multimodal_model()` 见 `docpage2md_app\aliyun_catalog.py:566`。
- `verify_all_capabilities()` 见 `docpage2md_app\aliyun_catalog.py:604`，更新 OpenAI text、OpenAI vision、DashScope multimodal 能力矩阵。
- `load_cache()` 与 `save_cache()` 分别见 `docpage2md_app\aliyun_catalog.py:745` 和 `docpage2md_app\aliyun_catalog.py:759`。

`cli.py` 中的模型目录命令由 `_handle_model_commands()` 处理，见 `docpage2md_app\cli.py:73`：

- `--refresh-models`：抓取候选并写入 `.cache\aliyun_model_catalog.json`。
- `--verify-models`：读取目录并用 API Key 探测模型能力，然后回写缓存。
- `--list-models` 和 `--list-all-models`：用 `_display_models_table()` 展示模型表，见 `docpage2md_app\cli.py:147`。

### 9.2 交互式模型选择

`docpage2md_app\model_settings.py:30` 的 `configure_models()` 是模型选择入口：
...
- Vision 展示由 `_print_vision_candidates()` 实现，见 `docpage2md_app\model_settings.py:156`。它使用 REC/CAN/FAIL 分层，并展示 OpenAI vision 与 DashScope multimodal 状态。
- Brain 展示由 `_print_brain_candidates()` 实现，见 `docpage2md_app\model_settings.py:199`。它展示 text 状态、价格、价格来源和 provider。

`_apply_record()` 见 `docpage2md_app\model_settings.py:263`，把 `ModelRecord` 应用到 `AppConfig`，包括 provider、model_id、base_url、api_key_env 和价格字段。

API Key 处理：

- `docpage2md_app\env.py` 通过 `get_env_value()` 读取环境变量。
- Windows 下优先读当前用户环境变量注册表，见 `docpage2md_app\env.py:12`。
- `_prompt_and_save_api_key()` 见 `docpage2md_app\model_settings.py:664`，可把用户粘贴的 key 写入当前用户环境变量；不写入仓库文件。
- `check_runtime_env()` 见 `docpage2md_app\files.py:44`，按当前 Vision/Brain provider 和 env 名称检查是否缺 key。
...
`docpage2md_app\config.py` 当前新增：

```text
third_party_models_path -> log\third_party_models.json
```

该属性位于 `docpage2md_app\config.py:109`，返回 `self.log_path / "third_party_models.json"`。

### 10.2 model_settings.py 从“自定义一次性模型”扩展为“第三方模型注册表”

当前 `docpage2md_app\model_settings.py` import 了 `third_party_models.py` 中的多项函数：
...
- `choose_model()`：`docpage2md_app\model_settings.py:93`。
- `_print_third_party_candidates()`：`docpage2md_app\model_settings.py:230`。
- `_select_third_party_model()`：`docpage2md_app\model_settings.py:319`。
- `_manage_third_party_models()`：`docpage2md_app\model_settings.py:339`。
- `_create_third_party_model_wizard()`：`docpage2md_app\model_settings.py:386`。
- `_bulk_import_third_party_models()`：`docpage2md_app\model_settings.py:425`。
- `_discover_and_import_models()`：`docpage2md_app\model_settings.py:454`。
- `_edit_third_party_model()`：`docpage2md_app\model_settings.py:514`。
- `_delete_third_party_model()`：`docpage2md_app\model_settings.py:558`。
- `_verify_registry_item()`：`docpage2md_app\model_settings.py:584`。
- `_apply_registry_item()`：`docpage2md_app\model_settings.py:286`。
...
`docpage2md_app\third_party_models.py` 是未跟踪新文件，当前职责是：
...
- `load_third_party_models()`：`docpage2md_app\third_party_models.py:20`。
- `save_third_party_models()`：`docpage2md_app\third_party_models.py:36`。
- `upsert_third_party_model()`：`docpage2md_app\third_party_models.py:47`。
- `delete_third_party_model()`：`docpage2md_app\third_party_models.py:63`。
- `registry_item_to_model_record()`：`docpage2md_app\third_party_models.py:72`。
- `filter_registry_models()`：`docpage2md_app\third_party_models.py:101`。
- `parse_bulk_models_text()`：`docpage2md_app\third_party_models.py:105`。
- `discover_openai_compatible_models()`：`docpage2md_app\third_party_models.py:148`。
- `_normalize_registry_item()`：`docpage2md_app\third_party_models.py:193`。
- `_parse_roles()`：`docpage2md_app\third_party_models.py:216`。
- `_dedupe_key()`：`docpage2md_app\third_party_models.py:259`。
...
1. `docpage2md_app\third_party_models.py` 是 untracked 文件。如果只提交 `config.py` 和 `model_settings.py`，运行时会因为 import 缺失而直接失败。
...
- 从 `docpage2md_app.cli` import `main`。
- 调用 `main()`，捕获 `KeyboardInterrupt` 后返回 130。

这是薄入口，业务逻辑都在 `docpage2md_app`。

### 11.2 `docpage2md_app\cli.py`

职责：CLI 参数、依赖检查、配置构造、模型目录命令、主交互流程、Rich 进度条、多文档页图片任务进程调度。

关键位置：

- `parse_args()`：`docpage2md_app\cli.py:12`。
- `ensure_dependencies()`：`docpage2md_app\cli.py:35`。
- `build_config()`：`docpage2md_app\cli.py:50`。
- `configure_stdio()`：`docpage2md_app\cli.py:66`。
- `_handle_model_commands()`：`docpage2md_app\cli.py:73`。
- `_display_models_table()`：`docpage2md_app\cli.py:147`。
- `main()`：`docpage2md_app\cli.py:208`。

设计特点：

- 把模型目录命令和普通转换流程合在一个入口。
- 使用 Rich `Live` 和 `Progress` 展示所有任务的进度。
- 使用 `ProcessPoolExecutor` 支持多个文档页图片任务并行，但默认 worker 为 1。
...
### 11.3 `docpage2md_app\pipeline.py`

职责：单个 DocPage2MD 图片任务的双阶段 pipeline 控制器。

关键函数：

- `process_single_docpage_task()`：`docpage2md_app\pipeline.py:10`。
- `_run_vision_stage()`：`docpage2md_app\pipeline.py:71`。
- `_run_brain_stage()`：`docpage2md_app\pipeline.py:139`。
...
### 11.4 `docpage2md_app\models.py`

职责：API 调用、流式响应收集、Vision/Brain worker、Markdown 清洗、API Key 设置。

关键函数：

- `retry_with_backoff()`：`docpage2md_app\models.py:26`。
- `call_aliyun_openai_chat()`：`docpage2md_app\models.py:81`。
- `call_aliyun_openai_vision()`：`docpage2md_app\models.py:125`。
- `_collect_openai_stream()`：`docpage2md_app\models.py:198`。
- `_collect_openai_sync()`：`docpage2md_app\models.py:228`。
- `run_stage_1_vision()`：`docpage2md_app\models.py:246`。
- `_run_openai_compatible_vision()`：`docpage2md_app\models.py:357`。
- `get_raw_content()`：`docpage2md_app\models.py:387`。
- `run_stage_2_brain_parallel()`：`docpage2md_app\models.py:395`。
- `sanitize_stage_2_markdown()`：`docpage2md_app\models.py:417`。
- `_run_dashscope_brain()`：`docpage2md_app\models.py:464`。
- `_run_openai_compatible_brain()`：`docpage2md_app\models.py:501`。
- `_run_deepseek_brain()`：`docpage2md_app\models.py:525`。
- `set_dashscope_api_key()`：`docpage2md_app\models.py:589`。
...
### 11.5 `docpage2md_app\prompts.py`

职责：集中存放两阶段 prompt。

关键常量：

- `PROMPT_STAGE_1_VISION`：`docpage2md_app\prompts.py:1`。
- `PROMPT_STAGE_2_BRAIN`：`docpage2md_app\prompts.py:26`。
...
### 11.6 `docpage2md_app\config.py`

职责：默认值与不可变配置对象。

关键内容：

- 默认模型和 provider：`docpage2md_app\config.py:6` 到 `docpage2md_app\config.py:9`。
- 默认输入/输出/log/cache 目录：`docpage2md_app\config.py:11` 到 `docpage2md_app\config.py:14`。
- 默认并发：`docpage2md_app\config.py:16` 到 `docpage2md_app\config.py:18`。
- 默认 thinking budget：`docpage2md_app\config.py:20` 到 `docpage2md_app\config.py:21`。
- API retry 默认值：`docpage2md_app\config.py:24` 到 `docpage2md_app\config.py:26`。
- API endpoint 和 env 名称：`docpage2md_app\config.py:29` 到 `docpage2md_app\config.py:33`。
- `AppConfig`：`docpage2md_app\config.py:41`。
- `model_settings_path`：`docpage2md_app\config.py:105`。
- `third_party_models_path`：`docpage2md_app\config.py:109`，当前未提交改动新增。
...
### 11.7 `docpage2md_app\cost.py`

职责：图片 token 估算、文本 token 成本估算、Rich 成本表展示。

关键函数：

- `calculate_image_tokens()`：`docpage2md_app\cost.py:4`。
- `estimate_price()`：`docpage2md_app\cost.py:24`。
- `estimate_text_cost()`：`docpage2md_app\cost.py:59`。
- `_resolve_tier_price()`：`docpage2md_app\cost.py:112`。
- `estimate_flat_price()`：`docpage2md_app\cost.py:132`。
- `show_cost_estimation()`：`docpage2md_app\cost.py:141`。
- `_find_model_record()`：`docpage2md_app\cost.py:246`。
...
### 11.8 `docpage2md_app\files.py`

职责：日志、输入目录检查、环境变量检查、文件扫描、自然排序、Markdown 合并、JSON 读写。

关键函数：

- `setup_logger()`：`docpage2md_app\files.py:13`。
- `ensure_input_folder()`：`docpage2md_app\files.py:33`。
- `check_runtime_env()`：`docpage2md_app\files.py:44`。
- `_required_api_envs()`：`docpage2md_app\files.py:80`。
- `natural_sort_key()`：`docpage2md_app\files.py:118`。
- `scan_docpage_folders()`：`docpage2md_app\files.py:123`。
- `extract_context()`：`docpage2md_app\files.py:139`。
- `merge_markdowns()`：`docpage2md_app\files.py:150`。
- `read_json()`：`docpage2md_app\files.py:169`。
- `write_json()`：`docpage2md_app\files.py:174`。
...
### 11.9 `docpage2md_app\model_settings.py`
...
### 11.10 `docpage2md_app\third_party_models.py`
...
默认单阶段 60 线程并发对 API 服务商是强压力。虽然 README 说明多个文档页图片任务默认不并行，但单个任务的页级并发仍可能触发：
...
2. `files.natural_sort_key()` 和 `scan_docpage_folders()`。
...
- `docpage2md_app\config.py` 中的 `third_party_models_path`。
- `docpage2md_app\model_settings.py` 中的交互流程修改。
...
当前 `DocPage2MD` 工作区已经从单纯图片目录输入，扩展为 MinerU 多格式解析 + Vision/Brain 双阶段精修的本地 CLI 工具。它已经具备端到端可用性、模型选择体验、成本预估、断点续跑、并发处理和日志记录。设计上最有价值的部分是把 MinerU layout/crop、视觉提取、结构化 IR、Markdown renderer 和审计报告拆开，并用缓存支撑长任务恢复。
