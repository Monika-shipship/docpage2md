# mineru-refine 与 png2md 对比及 png2md 精进路线图

研究对象：

- `D:\Repos\lab-python\工具\文档处理\mineru-refine`
- `D:\Repos\lab-python\png2md`

本报告依赖的既有研究报告：

- `D:\Repos\lab-python\png2md\docs\research\mineru-refine-research.md`
- `D:\Repos\lab-python\png2md\docs\research\png2md-research.md`

本报告只对比两份报告已经建立的事实，并针对少数关键点回看源码核对。没有重新做泛泛项目介绍，没有运行真实模型调用，也没有运行测试。当前仓库实际包名是 `ppt2md_app`，但下文按用户任务称为 `png2md` 或 `PPT2MD-V10`。

## 0. 研究边界与已验证事实

本报告写入目标是：

- `D:\Repos\lab-python\png2md\docs\research\comparison-and-png2md-roadmap.md`

本次只允许创建或写入这一份报告文件。当前主工作区已有未提交改动：

- `ppt2md_app\config.py`
- `ppt2md_app\model_settings.py`
- `ppt2md_app\third_party_models.py`
- `docs\research\mineru-refine-research.md`
- `docs\research\png2md-research.md`

本报告不会建议把这些未提交改动当作已经稳定发布的能力；只把它们视为当前分支中已经接入或正在接入的工程事实。

额外核对过的关键源码点包括：

- `ppt2md_app\pipeline.py`：`process_single_ppt_task()`、`_run_vision_stage()`、`_run_brain_stage()`。
- `ppt2md_app\models.py`：`run_stage_1_vision()`、`run_stage_2_brain_parallel()`、`sanitize_stage_2_markdown()`、错误字符串返回路径。
- `ppt2md_app\prompts.py`：两阶段 prompt 对单页边界、Figure 描述和 Markdown 输出的约束。
- `ppt2md_app\config.py`：默认并发、默认模型、`model_settings_path`、`third_party_models_path`。
- `ppt2md_app\model_settings.py` 与 `ppt2md_app\third_party_models.py`：第三方模型注册表已经被模型选择流程 import。
...
`mineru-refine` 和当前 `png2md` 都在解决“模型参与的文档到 Markdown 转换质量问题”，但两者所处抽象层和对 LLM 的授权边界完全不同。
...
当前 `png2md` 的核心是端到端本地 CLI 工作流：读取图片目录，Stage 1 Vision 生成自然语言 Raw Data，Stage 2 Brain 用五页窗口直接生成 `Slide_XX.md`，最后合并成 `{ppt_name}_FULL.md`。它已经有模型选择、成本估算、并发、日志和断点续跑，但结构化中间层较弱，最终 Markdown 的正确性主要依赖 prompt 与 `sanitize_stage_2_markdown()`，程序侧尚不能证明没有跨页泄漏、遗漏、误改或 API 错误字符串污染。

因此，`png2md` 不应该短期照搬 `mineru-refine` 的 Rust core、多语言绑定或 MinerU 字段规则。真正应迁移的是工程契约：
...
1. 都把 LLM/VLM 放进文档转换链路。
   ...
   - `png2md` 使用 Vision 模型做 OCR/公式/Figure 描述，再使用 Brain 模型生成 Markdown。

2. 都面对长文档和批处理。
   - `mineru-refine` 用 worklist、进度回调、cache key 和 report 支撑可审计长任务。
   - `png2md` 用图片目录扫描、页级并发、日志、Raw JSON 缓存、Slide 文件跳过和 session 恢复支撑长任务。

3. 都需要模型配置管理。
   - `mineru-refine` cache key 会把 refine logic version、模型、prompt version 和 opt-in flags 纳入输出身份。
   - `png2md` 当前已经有官方模型目录、阿里云模型缓存、DeepSeek 便利路径，以及未提交的第三方模型注册表。

4. 都需要错误可恢复。
   - `mineru-refine` 通过 fail-open 保守返回。
   - `png2md` 当前遇到 Vision 失败会给该页填入 `[Vision Failed]` 并继续，Stage 2 出错路径也多以字符串返回；这保证流程不中断，但还缺少“失败输出不可伪装成正常 Markdown”的闸门。

5. 都有输出 Markdown 的目标。
   - `mineru-refine` 的 Markdown 是从结构化 item 确定性渲染出来的派生产物。
   - `png2md` 的 Markdown 是 Brain 模型直接生成后轻量 sanitize 的主产物。

6. 都已经有“分阶段处理”的雏形。
   - `mineru-refine` 分为机械清洗、agent loop、opt-in garbled rewrite/degrade、opt-in confusion。
   - `png2md` 分为 Stage 1 Vision 和 Stage 2 Brain。

共同点说明：`png2md` 不需要推翻当前两阶段 pipeline，而是应该在两阶段之间和 Stage 2 之后补上结构化契约、报告、缓存指纹和质量闸门。

## 3. 核心差异

| 维度 | mineru-refine | 当前 png2md / PPT2MD-V10 | 对 png2md 的含义 |
|---|---|---|---|
| 输入 | MinerU `content_list` item 数组 | 图片目录中的 `.jpg/.jpeg/.png/.bmp/.webp` | png2md 必须处理 OCR/视觉理解源头，不能直接套 MinerU 的 item schema |
| 输出主合同 | 输入输出同 schema，返回 `items/provenance/report` | `Raw_XX.json`、`Slide_XX.md`、`FULL.md` | png2md 需要新增中间合同，不能只靠文件存在 |
| 中间表示 | `MineruItem` + `RefItem` + stable id | `raw_data_map: {slide_no: raw_text}`，Raw Data 是自然语言字符串 | png2md 的局部修复能力受限，应建立 Page IR / Block IR |
...
当前 `png2md` 的抽象层级从低到高是：

1. 图片任务：`scan_ppt_folders()` 返回 `{任务名: [图片路径]}`。
...
6. 汇总：`merge_markdowns()` 合并 `Slide_*.md` 为 `{ppt_name}_FULL.md`。
...
1. 局部修复能力不同。
   ...
   - 当前 `png2md` 如果发现一页 Markdown 有问题，通常只能重跑整页 Stage 2，甚至重跑 Stage 1。

2. 质量验证能力不同。
   - `mineru-refine` 可以在 op 落地时判断是否新增字符、破坏表格主体或产生非法几何。
   - 当前 `png2md` 只能做表层 sanitize，无法稳定判断“Target 页内容是否全部保留且没有混入前后页”。

因此，`png2md` 的优先精进方向不是继续强化 prompt，而是降低最终输出对“整页自由生成”的依赖。
...
### 5.2 当前 png2md 的 LLM 边界

当前 `png2md` 的 LLM 权力边界更宽：
...
### 5.3 png2md 需要的分层授权模型

`png2md` 不能简单采用 `mineru-refine` 的“核心层不新增字符”作为全局规则，因为它的输入是图片，图像理解本身会产生文本化描述，尤其 Figure Analysis 不是原图里的逐字文本。但可以按内容来源分层授权：
...
## 6. 应迁移到 png2md 的 mineru-refine 设计

以下设计应迁移，迁移的是思想和合同，不是照抄 Rust 代码。

### 6.1 稳定内部 ID

应迁移。`png2md` 的页、块、表格、公式、Figure 描述都应有稳定 ID，例如 `pg_0001`、`blk_0001_0003`、`tbl_0001_0001`。所有 refiner op 都引用 ID，而不是数组下标或 Markdown 行号。
...
应迁移。`png2md` 需要 Page IR / Block IR。即使 Stage 1 仍保留 Raw Text，也应并行产出或后处理出结构化 blocks。
...
应迁移。`png2md` 应先用规则发现疑点，例如空页、过短输出、API error 字符串、跨页内容泄漏、疑似标题误判、公式 delimiter 不平衡、Figure Analysis 未转为 note、表格行列异常，再决定是否调用 LLM。
...
应迁移。`png2md` 应定义 `Png2mdOp`，由 `apply_op_checked()` 或同等函数统一执行。最小 op 可包括：
...
应迁移，但要按来源分层。`png2md` 不应照搬“输出字符必须是输入字符子集”到所有块，而应定义：
...
应迁移。`png2md` 的 fail-open 不应表现为把错误字符串写成 `Slide_XX.md`。更合适的是：
...
应迁移。`mineru-refine` 的测试价值不在 Rust，而在 mock chat、mock vision、golden input、idempotence、fail-open 和 reject 测试。`png2md` 的模型调用必须抽象到可 mock，否则后续 refiner 无法稳定测试。

## 7. 不应迁移或不应立即迁移的设计

### 7.1 不应短期迁移 Rust core 与多语言绑定

当前 `png2md` 是 Python CLI 项目，主要模块都在 `ppt2md_app`。短期改成 Rust core 或 PyO3/napi 会增加构建、发布、调试和 Windows 环境复杂度。除非未来有明确多语言 SDK 或性能瓶颈，否则应先在 Python 中迁移架构契约。

### 7.2 不应照搬 MinerU 字段和 detector

`mineru-refine` 规则依赖 `type`、`text_level`、`table_body`、`table_caption`、`img_path`、`page_idx`、`bbox`、`list_items` 等 MinerU 字段。当前 `png2md` 没有这些字段。直接翻译 `detect.rs` 会产生错配。应先定义 `png2md` 自己的 Page IR，再设计 detector。

### 7.3 不应把“no new character”作为全局合同

`png2md` 的输入是图片，模型需要把图像转成文字、公式和 Figure 描述。Figure 描述本质是视觉解释，无法要求字符来自 Raw OCR。应把 no-new-char 改成来源分层合同，而不是全局规则。

### 7.4 不应一开始引入完整 agent loop

`mineru-refine` 的 agent loop 有 observe、多轮、并发、矛盾裁决、视觉裁决和振荡保护。`png2md` 当前没有结构化 IR 和 op executor，过早引入完整 loop 会把复杂度压在薄弱基础上。应先做 single-pass detector + op executor + mock tests，再考虑多轮 observe。

### 7.5 不应默认开启 OCR confusion 和视觉表格重转写

OCR 混淆、乱码表视觉重转写和降级在 `mineru-refine` 中都是 opt-in。`png2md` 的基础结构尚未稳定，不应默认打开高风险生成式修复。应先做审计和闸门，再以显式开关启用。
...
以下风险来自 `png2md-research.md` 和本次源码核对。
...
- `ppt2md_app\pipeline.py`
- `ppt2md_app\models.py`
- `ppt2md_app\prompts.py`
- `ppt2md_app\config.py`
- `ppt2md_app\files.py`
- 可新增 `ppt2md_app\manifest.py`

推荐接口/数据结构：

```python
PNG2MD_PIPELINE_VERSION = "png2md-pipeline-v1"
...
{
  "schema_version": 1,
  "pipeline_version": "png2md-pipeline-v1",
  "ppt_name": "...",
  "pages": [
    {
      "slide_no": 1,
...
```text
{output_folder}\{ppt_name}\run_report.json
```
...
def render_full_markdown(pages: list[PageIR], ppt_name: str) -> str:
    ...
```
...
### v3.2 Png2mdOp 与 `apply_op_checked`
...
- 可新增 `ppt2md_app\refiner.py`
- `ppt2md_app\models.py`
- `ppt2md_app\detect.py`
- `ppt2md_app\ops.py`
...
- 可新增 `ppt2md_app\confusion.py`
- 可新增 `ppt2md_app\tables.py`
- `ppt2md_app\ir.py`
- `ppt2md_app\ops.py`
- `ppt2md_app\report.py`
...
- `pytest`
- import smoke test：`python -m ppt2md_app --help` 或等效。
...
这个架构保留当前 `png2md` 最有价值的两阶段思路，但把最终输出从“模型直接写文件”转为“结构化 IR + renderer + 可选受限 refiner”。
...
- `D:\Repos\lab-python\png2md\ppt2md_app\pipeline.py`
- `D:\Repos\lab-python\png2md\ppt2md_app\models.py`
- `D:\Repos\lab-python\png2md\ppt2md_app\prompts.py`
- `D:\Repos\lab-python\png2md\ppt2md_app\config.py`
- `D:\Repos\lab-python\png2md\ppt2md_app\files.py`
- `D:\Repos\lab-python\png2md\ppt2md_app\model_settings.py`
- `D:\Repos\lab-python\png2md\ppt2md_app\third_party_models.py`
- `D:\Repos\lab-python\png2md\ppt2md_app\cost.py`
