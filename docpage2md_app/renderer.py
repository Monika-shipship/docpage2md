...
def _render_figure_note(block: Dict[str, Any]) -> str:
    ...
    details = _figure_detail_lines(
        ...
        uncertain=bool(block.get("unrecognizable") or figure.get("unrecognizable") or _has_figure_uncertain_marker(text)),
    )
    if details:
        rendered.append(_render_collapsible_details("图示识别内容", details))
    return "\n\n".join(rendered)


def _figure_detail_lines(
    *,
    lines: list[str],
    figure: dict,
    labels: list[str],
    ...
    if figure_type and figure_type != "unknown":
        details.append(f"- 类型：{figure_type}")
    if lines:
        if len(lines) == 1:
            details.append(f"- 说明：{lines[0]}")
        else:
            details.append("- 说明：")
            details.extend(f"  - {line}" for line in lines)
    if labels:
        details.append("- 可见标签：" + "，".join(str(label) for label in labels[:16]))
    if relations:
        details.append("- 主要关系：" + "；".join(str(relation) for relation in relations[:8]))
    if linked_blocks:
        details.append("- 与正文/公式的关联：" + "，".join(str(block_id) for block_id in linked_blocks[:12]))
    if uncertainties:
        details.append("- 不确定点：" + "；".join(str(item) for item in uncertainties[:8]))
    elif uncertain:
        details.append("- 不确定点：图示不可可靠识别。")
    return details
