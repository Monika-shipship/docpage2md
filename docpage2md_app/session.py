...
from .files import scan_docpage_folders
...
def interactive_setup(console, api_key, config: AppConfig):
    ...
    all_tasks = scan_docpage_folders(config.input_folder)
    if not all_tasks:
        console.print(f"[bold red]❌ 未在 {config.input_folder} 中发现子文件夹。[/]")
        return None

    doc_names = list(all_tasks.keys())
    ...
    if selection.lower() == "a":
        selected_names = doc_names
    else:
        try:
            for idx in [int(x) for x in selection.split(",") if x.strip()]:
                if 1 <= idx <= len(doc_names):
                    selected_names.append(doc_names[idx - 1])
        except Exception:
            return None
